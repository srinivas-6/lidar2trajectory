...
class CameraPoseLoss(nn.Module):
    ...
    def forward(self, est_pose, gt_pose):
            """
            Forward pass
            :param est_pose: (torch.Tensor) batch of estimated poses, a Nx3 tensor
            :param gt_pose: (torch.Tensor) batch of ground_truth poses, a Nx3 tensor
            :return: camera pose loss
            """
            # Position loss
            l_x = torch.norm(gt_pose[:, 0:2] - est_pose[:, 0:2], dim=1, p=self.norm).mean()
            # Orientation loss (normalized to unit norm)
            l_q = torch.norm(F.normalize(gt_pose[:, 2:], p=2, dim=1) - F.normalize(est_pose[:, 2:], p=2, dim=1),
